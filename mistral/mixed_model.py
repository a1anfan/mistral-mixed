...
class MixedTransformer(nn.Module):
    def __init__(
        ...
        layers = [MixedTransformerBlock(args=args) for _ in range(args.n_layers)]
    ...
    def forward_partial(
        ...
        if self.pipeline_rank == 0:
            ...
            if token_weights is not None:
                h = (token_weights.half() @ self.tok_embeddings.weight).unsqueeze(1)
            else:
    ...
    def forward(
        ...
        h = self.forward_partial(input_ids, seqlens, cache=cache, token_weights=token_weights)
        if self.pipeline_rank < self.num_pipeline_ranks - 1:
